...
export function DashboardPage() {
  ...
  const dashboardBrief = summary?.ai_brief ?? null;
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
            ...
            {aiResult.source && (
              <p className="text-xs text-slate-600 mt-2 text-right">
                {aiResult.source === 'n8n_gpt'
                  ? `✦ Generated by ${aiResult.model_used ?? 'LLM'} via n8n`
      ...
      <div className="card">
        <div className="flex items-start justify-between mb-4">
          ...
          {dashboardBrief?.risk_level && (
            <span
              className={`px-2.5 py-1 rounded-md text-xs font-bold tracking-wide ${
                dashboardBrief.risk_level === 'LOW'
                  ? 'bg-green-600 text-white'
                  : dashboardBrief.risk_level === 'MODERATE'
                    ? 'bg-amber-600 text-white'
                    : dashboardBrief.risk_level === 'HIGH'
        ...
        {dashboardBrief ? (
          <div className="space-y-3">
            ...
            <div className="rounded-lg border border-slate-700 bg-slate-900/60 p-3">
              ...
              <p className="text-sm text-slate-200">{dashboardBrief.key_action}</p>
            </div>
            <p className="text-xs text-slate-500 text-right">
              ✦ Generated by {dashboardBrief.model_used} ({dashboardBrief.source})
            </p>
          </div>
        ) : (
          <div className="text-center py-8 text-slate-500">
            <p>No live AI brief available</p>
            <p className="text-sm mt-1">Run analytics to generate the latest risk brief</p>
          </div>
        )}
      </div>
